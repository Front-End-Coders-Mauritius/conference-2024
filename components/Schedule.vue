...
const agendaContentRef = ref(null)
...
onMounted(() => {
  ...
  interval = window?.setInterval(() => currentDateTime.value = dayjs(), 1000)
})

onBeforeMount(() => window?.clearInterval(interval))

function getIntervalDateTime(date: string, time: string, intervalInMins: number, operation: 'add' | 'subtract') {
  const parsedTime = dayjs(`${date}T${time}`, 'YYYY-MM-DDTHH:mm')
...
const timeintervals = computed(() => {
  ...
  const startTimeInterval = getIntervalDateTime(selectedDate.value, startTime, props.intervalInMins, 'subtract')
  const endTimeInterval = getIntervalDateTime(selectedDate.value, endTime!, props.intervalInMins, 'add')
  const intervals = []
...
const currentTimeGridRow = computed(() => {
  const agendaStartDateTime = timeintervals.value.at(0)
  const agendaEndDateTime = timeintervals.value.at(-1)
  const isCurrentDateInAgenda = currentDateTime.value.isBetween(agendaStartDateTime, agendaEndDateTime, null, '[]')

  if (!isCurrentDateInAgenda)
    return null

  return getGridRow(selectedDate.value, currentDateTime.value.format('HH:mm'))
})

function getGridRow(date: string, time: string) {
  const timeFromIntervalInMins = (dayjs(`${date}T${time}`, 'YYYY-MM-DDTHH:mm').diff(timeintervals.value.at(0)!, 'minute'))
  const rowOffset = 2
...
<template>
  <div class="bg-white py-8 rounded-lg">
    ...
    >
      <TabsList class="max-w-full justify-start bg-gray-50 overflow-x-auto">
        <TabsTrigger
          v-for="date in Object.keys(sessionsByDate)"
          :key="date"
          :value="date"
          class="grow data-[active=true]:text-white data-[active=true]:bg-blue-900 hover:data-[active=false]:bg-gray-100 motion-safe:transition-all"
        >
          {{ getDay(date) }}
        </TabsTrigger>
      </TabsList>
      ...
      >
        <div
          ...
          class="sticky top-0 row-start-1 row-end-2 col-start-1 -col-end-1 mb-5 bg-white border border-gray-100 rounded-md shadow-md z-[3]"
          aria-hidden="true"
        ></div>
        <AgendaRoom
          v-for="(room, index) in ROOMS"
          :key="room"
          class="sticky top-0 row-start-1 row-end-2 mb-5 p-2 uppercase text-md text-center text-blue-900 rounded-md z-[3]"
          :style="{
        ...
        >
          ...
          <AgendaTimeIntervalLine
            class="absolute [--height:2px] h-[--height] left-100% top-1/2 -translate-y-1/2 bg-yellow-300/50 pointer-events-none"
        ...
        <AgendaSession
          ...
          :link-class="cn(
            ...
            'h-auto p-0 rounded-lg bg-gray-50/25 hover:bg-gray-50 hover:shadow-lg focus-visible:bg-gray-50 focus-visible:shadow-lg motion-safe:transition',
          ...
          :style="{
            gridRowStart: getGridRow(selectedDate, session.time.start),
            gridRowEnd: getGridRow(selectedDate, session.time.end),
        ...
        <AgendaCurrentTime
          v-if="currentTimeGridRow"
          class="relative col-start-1 -col-end-1 z-[2] motion-safe:transition-all"
          :style="{
            gridRowStart: currentTimeGridRow,
            gridRowEnd: currentTimeGridRow + 1,
          }"
        >
          <AgendaCurrentTimeLine
            :current-date-time="currentDateTime"
            class="absolute [--height:4px] h-[--height] -top-[--height] right-0 -translate-y-1/2 bg-red-500/50 cursor-pointer hover:h-[12px] hover:-top-[8px] hover:bg-red-500/30 motion-safe:transition-all"
            :style="{
              width: `${roomsContainerWidth}px`,
            }"
          />
          <AgendaCurrentTimeOverlay
            class="absolute h-[100000vmax] right-0 bottom-0 bg-red-400/5 pointer-events-none"
            :style="{
              width: `${roomsContainerWidth}px`,
            }"
          />
